...
async function main() {
    ...
    const scenario = [
        ...
        },
        {
            'userData': prefixes + 'ex:Jacky a o-persoon:Inwoner ; \n' +
                'ex:hasDesign true ; \n' +
                'ex:hasDocumentsForBuildingPermitApplication true ; \n' +
                'ex:buildingPermitApplicationReady true .',
            'comment': '4. No further steps need to be taken.',
        }
    ];
    ...
    for (const {userData, comment} of scenario) {
        ...
        // write user profile
        await writeFile(path.resolve(basePath, config.personalInfo), userData, 'utf8');
